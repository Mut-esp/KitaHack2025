...
interface WorkShopEventPageProps {
  title: string;
  subtitle: string;
  date: string;
  time: string;
  ...
  // GDG Logos Section
  gdgLogos: { src: string; alt: string }[];
...
const WorkShopEventPage: React.FC<WorkShopEventPageProps> = ({
  title,
  subtitle,
  date,
  time,
  ...
  return (
    <main className="container mx-auto flex flex-col justify-center items-center gap-40 mt-20">
      {/* Hero Section */}
      <div className="container mx-auto my-20 flex flex-col items-center lg:flex-row px-10 md:px-20 lg:px-8 text-center lg:text-left max-w-6xl">
        <div className="lg:w-[450px] lg:min-w-[550px]">
          ...
          <Link href={rsvpLink} target="_blank" rel="noopener noreferrer">
            <button className="lg:mt-8 lg:mb-10 my-10 bg-gradient-to-r from-[#70dbf7] to-[#60d7f6] text-white font-semibold py-4 px-6 rounded shadow hover:scale-110 transition">
              RSVP Now
            </button>
          </Link>
        </div>
        <div className="mb-12 lg:mb-0">
          <Image src={eventImage} alt="Event Banner" width={500} height={300} className="rounded-lg shadow-2xl p-8" />
        </div>
      </div>


      {/* GDG LOGO */}
      <section className="container flex lg:justify-between mx-auto flex-col gap-12 lg:gap-0 items-center lg:flex-row">
      ...
      <section className="w-full flex flex-wrap justify-center items-center pt-10 px-4">
        ...
        {speakers && speakers.map((speaker, index) => (
          <div
            key={index}
            className="relative w-80 p-4 my-14 mx-6 bg-white rounded-lg flex flex-col justify-center items-center shadow-md dark:bg-gray-700"
          >
